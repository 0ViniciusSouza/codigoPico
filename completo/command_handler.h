...
namespace ch {
  ...
  command request2command(int my_node_index, String request){
    int k = 0;

    for(int i = 0; i < 32; i++){
      if(request.startsWith((String)commands[1][i])){
        k = i;
        break;
      }
    }
    ...
    if(x_type[1][k] == I_){
      id = request.substring(3).toInt();
    } 
    else if(x_type[1][k] == I_VAL_){
      id = request.substring(1,3).toInt();
      val = request.substring(3).toInt();
    }
    else if(x_type[1][k] == X_I_1){
      id = request.substring(3).toInt();
    }

    else if(x_type[1][k] == X_I_2){
      id = request.substring(5).toInt();
    }
  ...
  command response2command(int my_node_index, String request, int fromNode, String response){
    int k = 0;
  ...
  command frame2command(can_frame canMsgRx){
    command can_command{"None",0,0,0,0};
    ...
    else if (msg_type == X_I_1 ||msg_type == X_I_2){
      char x;
