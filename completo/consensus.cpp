...
float evaluate_cost(struct Consenso_no c_node, float d[], float rho, int vector_size){
    ...
    for(int i = 0; i< vector_size; i++){
        prod_int += c_node.c_c[i]*d[i];
        prod_int2 += c_node.c_y[i]*(d[i]-c_node.c_d_av[i]);
        norm += (d[i]-c_node.c_d_av[i])*(d[i]-c_node.c_d_av[i]);
    }
...
bool check_feasibility(struct Consenso_no c_node, float d[], int vector_size){
    ...
    if (d[c_node.c_id] < 0.-tol) return false;
    if (d[c_node.c_id] > 100.0 + tol) return false;
    float d_k{0};
    float L_o_t{0};
    for(int i=0; i<vector_size;i++){
        d_k += d[i]*c_node.c_k[i];
    }
    L_o_t += c_node.c_L - c_node.c_o - tol; 
...
float consensus_iterate(struct Consenso_no c_node, float rho, int size_array, float d[]){
    ...
    for(int i=0; i<size_array;i++){
        z[i]= rho*c_node.c_d_av[i]-c_node.c_y[i] - c_node.c_c[i];
    }
    for(int i=0; i<size_array;i++){
        d_u[i]=  (1/rho)*z[i];
    }
    sol_unconstrained = check_feasibility(c_node, d_u, size_array);
    if(sol_unconstrained){
        //REVISE: IF UNCONSTRAINED SOLUTION EXISTS, THEN IT IS OPTIMAL
        //NO NEED TO COMPUTE THE OTHER
            cost_unconstrained = evaluate_cost(c_node, d_u, rho, size_array);
    ...
    float prod_int{0};
    for(int i =0; i<size_array; i++){
        prod_int+= z[i]*c_node.c_k[i];
    }
    for(int i =0; i<size_array;i++){
        d_bl[i] = 1/rho*z[i] - c_node.c_k[i]/c_node.c_n*(c_node.c_o - c_node.c_L + (1/rho)*prod_int);
    }
    //check feasibility of minimum constrained to linear boundary
    sol_boundary_linear = check_feasibility(c_node, d_bl, size_array);
    //compute cost and if best store new optimum
    if(sol_boundary_linear){
        cost_boundary_linear = evaluate_cost(c_node, d_bl, rho, size_array);
    ...
    sol_boundary_0 = check_feasibility(c_node, d_b0, size_array);
    if(sol_boundary_0){
        cost_boundary_0 = evaluate_cost(c_node, d_b0, rho, size_array);
    ...
    d_b1[c_node.c_id] = 100;
     //check feasibility of minimum constrained to 100 boundary
    sol_boundary_100 = check_feasibility(c_node, d_b1, size_array);
    if(sol_boundary_100){
        cost_boundary_100 = evaluate_cost(c_node, d_b1, rho, size_array);
    ...
    float sub{0};
    for(int i=0;i<size_array;i++){
        sub+=z[i]*c_node.c_k[i];
    }
    for(int i =0; i<size_array; i++){
        d_l0[i] = (1/rho)*z[i] - (1/c_node.c_m)*c_node.c_k[i]*(c_node.c_o - c_node.c_L) + (1/rho/c_node.c_m)*c_node.c_k[i]*(c_node.c_k[c_node.c_id]*z[c_node.c_id]- sub);
    }
    d_l0[c_node.c_id] = 0;
    //check feasibility of minimum constrained to linear and 0 boundary
    sol_linear_0 = check_feasibility(c_node, d_l0, size_array);
    //compute cost and if best store new optimum
    if(sol_linear_0){
        cost_linear_0 = evaluate_cost(c_node,d_l0,rho,size_array);
    ...
    for(int i =0; i<size_array; i++){
        d_l1[i] = (1/rho)*z[i] - (1/c_node.c_m)*c_node.c_k[i]*(c_node.c_o - c_node.c_L + 100*c_node.c_k[c_node.c_id]) + (1/rho/c_node.c_m)*c_node.c_k[i]*(c_node.c_k[c_node.c_id]*z[c_node.c_id]- sub);
    }
    d_l1[c_node.c_id] = 100;
    //check feasibility of minunum constrained to linear and 0 boundary
    sol_linear_100 = check_feasibility(c_node, d_l1, size_array);
    //compute cost and if best store new optimum
    if(sol_linear_100){
        cost_linear_0 = evaluate_cost(c_node,d_l1,rho, size_array);
        if(cost_linear_0 < cost_best){
            for(int i =0; i<size_array; i++){
              d_best[i] = d_l1[i];
            }
...
// float consensus(struct c_node c_nodep, float rho, int maxiter, bool data_available, uint8_t ad1, uint8_t ad2, unsigned long time_to_write, unsigned long write_delay, MCP2515::ERROR err, struct can_frame canMsgTx, struct can_frame canMsgRx, int states[3]){
//   float d1[3]{0,0,0};
//   float d2[3]{0,0,0};
//   float d3[3]{0,0,0};
//   for(int i =0; i<maxiter; i++){
//     //minimize augmented lagragian with respect to di for the current values of yi and d_av
//     float cost1 = consensus_iterate(c_nodep, rho, 3, d1);
//     for(int j=0; j<3; j++){
//       c_nodep.d[j] = d1[j];
//     }
//     //exchanges solution 
...
//             //creates the message
//             unsigned long message = c_nodep.d[msg_sent];
//             canMsgTx.can_id = c_node_address;
//             canMsgTx.can_dlc = 5;
//             uint8_t command = 0b11100;
//             uint8_t c_node_pos;
//             if(msg_sent == 0) c_node_pos= 0x0;
//             else if(msg_sent == 1) c_node_pos = 0x1;
//             else if (msg_sent == 2) c_node_pos = 0x2;
//             uint8_t response = 0x0;
//             uint8_t iteration = (byte)i;
//             int int_b_value = c_nodep.d[msg_sent]*1000;
...
//             canMsgTx.data[0] = ( (c_node_pos << 6))|((response << 5)) |(command);
...
//             // Serial.print(" from c_node ");
//             // Serial.print( c_node_address, HEX );
...
//           interrupts();
//           //receives data from different c_node
...
//       }
//     }
//   //compute the average of each c_node's solution
//   for(int j=0; j<3; j++){
//       c_nodep.d_av[j] = (c_nodep.d[j]+d2[j]+d3[j])/3;
//     }
//   //update the Lagrange multipliers
//     for(int j=0; j<3; j++){
//       c_nodep.y[j] = c_nodep.y[j] + rho*(c_nodep.d[j]-c_nodep.d_av[j]);
//     }
//   }
//   //solucao dos duty cycles
//   Serial.println("Solucao final dos duty cycles: ");
//     for(int j=0;j<3;j++){
//       Serial.println(c_nodep.d_av[j]);
//     }

//   //solucao que indica a referencia que cada no precisa seguir
//   float reference = 0;
//   //mulitiplicar K*d+o
//   reference = c_nodep.k[0]*c_nodep.d_av[0] + c_nodep.k[1]*c_nodep.d_av[1] + c_nodep.k[2]*c_nodep.d_av[2] + c_nodep.o; 
//   Serial.println();
